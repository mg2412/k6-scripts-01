...
import { check } from 'k6';  // Add Counter
import { Counter } from 'k6/metrics';  // ✅ Correct module
import encoding from 'k6/encoding';

// Add these counters
const loginFailures = new Counter('login_failures');
const loginSuccesses = new Counter('login_successes');
const loginErrors = new Counter('login_errors_detailed');
...
function runFlow(role, paths, creds) {
  ...
  // Track login response status
  if (loginRes.status !== 200) {
    loginFailures.add(1);
    console.error(`[${role}] Login failed: Status ${loginRes.status} for ${creds.username}`);
    console.error(`Response body: ${loginRes.body}`);
  } else {
    loginSuccesses.add(1);
    console.log(`[${role}] Login successful for ${creds.username}`);
  }
